// PySQL-Sync Interactive Dashboard Engine
...
function initNavigation() {
  ...
  navLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      // Initialize map on tab switch to ensure correct dimensions
...
async function loadSystemStatus() {
  try {
    ...
    document.getElementById('total-records-count').innerText = data.total_records.toLocaleString();
...
function setupAiAssistant() {
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      insightsBox.innerText = `Error: ${err.message}`;
    } finally {
      btn.disabled = false;
...
async function initGeospatialMap() {
  ...
  try {
    const res = await fetch('/api/geo/density');
    ...
    // Fetch and draw top logistics routes
    const routeRes = await fetch('/api/geo/routes');
...
async function loadLogisticsDelay() {
  try {
    const res = await fetch('/api/ml/delay');
    const data = await res.json();
...
async function loadClvAndChurn() {
  try {
    const res = await fetch('/api/ml/clv');
    const data = await res.json();
    if (data.status !== 'success') return;
...
function setupQuerySelector() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(`/api/analytics/query/${qId}`);
      const data = await res.json();
...
async function loadRFMSegments() {
  try {
    const res = await fetch('/api/ml/rfm');
    const data = await res.json();
    if (data.status !== 'success') return;
...
async function loadMarketBasket() {
  try {
    const res = await fetch('/api/ml/basket');
    const data = await res.json();
    if (data.status !== 'success') return;
